...
for batch_counter, problem in enumerate(suite):  # this loop may take hours or days...
    ...
    while evalsleft() > 0 and not problem.final_target_hit:
        ...
        elif fmin is pyswarm.pso:  # cma.fmin2:
            fmin(problem,problem.lower_bounds,problem.upper_bounds,maxiter=100)
        elif fmin is scipy.optimize.fmin_cobyla:
            fmin(problem, propose_x0(), lambda x: -problem.constraint(x), maxfun=evalsleft(),
                 disp=0, rhoend=1e-9)
        elif fmin is scipy.optimize.differential_evolution:
            bounds = [x for x in zip(problem.lower_bounds, problem.upper_bounds)]
            
            fmin(problem,bounds,maxiter=200)
